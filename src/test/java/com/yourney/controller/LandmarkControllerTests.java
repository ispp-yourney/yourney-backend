...
class LandmarkControllerTests {
	...
	void setup() {
		...
		given(this.landmarkService.existsActivityByLandmarkId((long) LandmarkControllerTests.TEST_LANDMARK_ID4))
				.willReturn(true);
		given(this.landmarkService.findById((long) LandmarkControllerTests.TEST_LANDMARK_ID6))
				.willReturn(Optional.of(l6));
		given(this.landmarkService.findById((long) LandmarkControllerTests.TEST_LANDMARK_ID4))
				.willReturn(Optional.of(l4));
		given(this.landmarkService.findById((long) LandmarkControllerTests.TEST_LANDMARK_ID))
				.willReturn(Optional.of(l1));
	...
	void testListCountries() throws Exception {
		this.mockMvc.perform(get("/landmark/country/list"))
				...
				.andExpect(jsonPath("$", isA(ArrayList.class))).andExpect(jsonPath("$[0]", is("España")))
	...
	void testListCitiesByCountry() throws Exception {
		this.mockMvc.perform(get("/landmark/country/{name}/city/list", TEST_LANDMARK_COUNTRY))
				...
				// Validate the returned fields
				.andExpect(jsonPath("$", isA(ArrayList.class))).andExpect(jsonPath("$[0]", is("Sevilla")));
	}
	...
	void testSearchByPropertiesLandMark() throws Exception {
		this.mockMvc
				.perform(get("/landmark/search?page={page}&size={size}&country={country}&city={city}&name={name}",
	...
	void testShowLandMark() throws Exception {
		this.mockMvc.perform(get("/landmark/show/{id}", TEST_LANDMARK_ID))
				...
				// Validate the returned fields
				.andExpect(jsonPath("$.id", is(1))).andExpect(jsonPath("$.name", is("Giralda")))
				.andExpect(jsonPath("$.description", is("lorem ipsum"))).andExpect(jsonPath("$.price", is(0.0)))
				.andExpect(jsonPath("$.country", is("España"))).andExpect(jsonPath("$.city", is("Sevilla")));
	}
	...
	void testUpdateLandmark() throws Exception {
		...
		this.mockMvc
				.perform(put("/landmark/update").contentType(MediaType.APPLICATION_JSON)
	...
	void testDeleteLandmarkWithActivity() throws Exception {
		...
		this.mockMvc.perform(delete("/landmark/delete/{id}", TEST_LANDMARK_ID4))
				...
				.andExpect(jsonPath("$.text",
						is("El punto de interés se encuentra asociado con al menos una actividad.")));
	}
	...
	void testUpgradeNegativeDaysLandmark() throws Exception {
		...
		this.mockMvc
				.perform(get("/landmark/upgrade?landmarkId={landmarkId}&subscriptionDays={subscriptionDays}",
						TEST_LANDMARK_ID4, -33))
